...
class DirectionScore(BaseModelWithDatetime):
    """方向评分结果"""
    direction: str
    search_quality_score: float = Field(ge=0, le=100)  # 搜索质量评分 (0-100)
    content_depth_score: float = Field(ge=0, le=100)   # 内容深度评分 (0-100)
    technical_score: float = Field(ge=0, le=100)        # 技术指标评分 (0-100)
    overall_score: float = Field(ge=0, le=100)         # 综合评分 (0-100)
    score_breakdown: Dict[str, Any] = Field(default_factory=dict)  # 评分详情
    timestamp: datetime = Field(default_factory=datetime.now)

class OptimizedReport(BaseModelWithDatetime):
    """优化报告结构"""
    topic: str
    best_direction: str
    best_direction_score: float
    executive_summary: str
    detailed_analysis: Dict[str, Any]  # 最佳方向详细分析
    brief_analyses: List[Dict[str, Any]]  # 其他方向简要分析
...
class OptimizedWorkflowState(TypedDict):
    ...
    # 报告优化阶段
    optimized_report: Optional[OptimizedReport]
