...
from storage_models import IntegratedWorkflowRecord, SearchRoundRecord, OptimizedReport
...
智能分析系统
    ...
    def format_optimized_report(self, optimized_report: OptimizedReport, recipient: str = "用户") -> str:
        """格式化优化报告为邮件"""
        try:
            # 获取基本信息
            ...
            executive_summary = optimized_report.executive_summary
            detailed_analysis = optimized_report.detailed_analysis
            brief_analyses = optimized_report.brief_analyses
            comprehensive_recommendations = optimized_report.comprehensive_recommendations
            technical_stats = optimized_report.technical_stats
            ...
            email_parts = [
                ...
                "您好！",
                "",
                f"以下是关于'{topic}'的优化分析报告，经过智能评分和报告优化处理。",
                "",
                "## 📋 分析概览",
                f"- **分析主题**：{topic}",
                f"- **最佳方向**：{best_direction}（评分：{best_score:.1f}分）",
                f"- **分析维度**：{len(brief_analyses) + 1}个方向",
                f"- **生成时间**：{optimized_report.generated_at.strftime('%Y年%m月%d日 %H:%M:%S')}",
                "",
                "## 📊 执行摘要",
                executive_summary,
                "",
                "## 🔍 最佳方向详细分析",
                "",
                f"### {best_direction}（评分：{best_score:.1f}分）",
                "",
                f"**方向说明**：{detailed_analysis.get('direction_rationale', '该方向的重要性和价值')}",
                "",
                f"**问题分析**：",
                ""
            ]
            ...
            # 添加问题分析
            questions_analysis = detailed_analysis.get('questions_analysis', [])
            for i, question_analysis in enumerate(questions_analysis, 1):
                email_parts.extend([
                    f"**{i}. {question_analysis['question_text']}**",
                    f"   - 搜索状态：{question_analysis['search_status']}",
                    f"   - 结果数量：{question_analysis['results_count']}个",
                    f"   - 摘要：{question_analysis['summary'][:200]}{'...' if len(question_analysis['summary']) > 200 else ''}",
                    ""
                ])
                
                # 添加关键点
                if question_analysis.get('key_points'):
                    email_parts.append(f"   - 关键点：")
                    for point in question_analysis['key_points'][:3]:
                        email_parts.append(f"     • {point}")
                    email_parts.append("")
            
            # 添加关键发现
            key_findings = detailed_analysis.get('key_findings', [])
            if key_findings:
                email_parts.extend([
                    "**关键发现**：",
                    ""
                ...
                for i, finding in enumerate(key_findings[:8], 1):
                    email_parts.append(f"{i}. {finding}")
                email_parts.append("")
            
            # 添加详细洞察
            detailed_insights = detailed_analysis.get('detailed_insights', [])
            ...
            if brief_analyses:
                ...
                for brief_analysis in brief_analyses:
                    email_parts.extend([
                        f"### {brief_analysis['direction_name']}（评分：{brief_analysis['score']:.1f}分）",
                        "",
                        brief_analysis['summary'],
                        ""
                    ])
                    
                    # 添加关键洞察
                    if brief_analysis.get('key_insights'):
                        email_parts.append("**关键洞察**：")
                        for insight in brief_analysis['key_insights'][:3]:
                            email_parts.append(f"• {insight}")
                        email_parts.append("")
                    
                    # 添加代表性链接
                    if brief_analysis.get('representative_links'):
                        email_parts.append("**主要信息来源**：")
                        for link in brief_analysis['representative_links'][:3]:
                            email_parts.append(f"• {link['title']}")
                        email_parts.append("")
            ...
            if comprehensive_recommendations:
                ...
                for i, recommendation in enumerate(comprehensive_recommendations, 1):
                    email_parts.append(f"{i}. {recommendation}")
                email_parts.append("")
            ...
            if technical_stats:
                email_parts.extend([
                    "## 📈 技术统计信息",
                    "",
                    f"- 总分析方向：{technical_stats.get('scoring_stats', {}).get('total_directions', 0)}个",
                    f"- 平均评分：{technical_stats.get('scoring_stats', {}).get('average_score', 0):.1f}分",
                    f"- 搜索成功率：{technical_stats.get('search_stats', {}).get('success_rate', 0):.1%}",
                    f"- 总搜索结果：{technical_stats.get('search_stats', {}).get('total_results', 0)}个",
                    f"- 平均处理时间：{technical_stats.get('search_stats', {}).get('average_processing_time', 0):.2f}秒",
                    ""
                ])
